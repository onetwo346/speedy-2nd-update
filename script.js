...
const loadOrders = () => {
    ...
    orders.clear(); // Clear existing orders to avoid duplicates
    if (storedOrders) {
    ...
    console.log("Orders loaded:", Array.from(orders.entries())); // Debug log
};
...
const updateOrderStatus = (referenceNumber, newStatus, photo = null) => {
    ...
    if (photo) order.deliveryPhoto = photo; // Store photo URL or base64 data
    orders.set(referenceNumber, order);
    saveOrders(referenceNumber);
    // TODO: In a real app, update the status in a backend database
    // Example: db.collection("orders").doc(referenceNumber).update({ status: newStatus, deliveryPhoto: photo });
};
...
const handleTrackOrder = () => {
    ...
    const pollForUpdates = () => {
        const interval = setInterval(() => {
            loadOrders(); // Reload orders to catch updates
            const order = orders.get(referenceNumber);
