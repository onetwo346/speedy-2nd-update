...
const saveOrders = (referenceNumber) => {
    ...
    // Notify listeners (customer page) of the update
...
const updateTrackResult = (referenceNumber) => {
    ...
    // Update order result if visible
...
if (document.getElementById("order-form")) {
    document.addEventListener("DOMContentLoaded", () => {
        ...
        // Listen for order updates from driver
        window.addEventListener("orderUpdated", (event) => {
            const referenceNumber = event.referenceNumber;
            if (getElement("track-input")?.value.trim() === referenceNumber) {
                updateTrackResult(referenceNumber);
            }
            const statusElement = getElement(`status-${referenceNumber}`);
            if (statusElement) {
